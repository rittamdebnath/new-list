...
function removeItem() {
  ...
  req.open('DELETE', '/todo/list/' + taskId);
...
function completeItem() {
  ...
  var taskId = item.getAttribute('data-id');
  // console.log(item + ' ' + parent + ' ' + id + ' ' + taskId)

  // Check if the item should be added to the completed list or to re-added to the todo list
  var target = (id === 'todo') ? document.getElementById('completed') : document.getElementById('todo');

  var req = new XMLHttpRequest();
  req.open('PATCH', '/todo/list/' + taskId);
  req.setRequestHeader('Content-Type', 'application/json');
...
function addItemToDOM(task, completed) {
  ...
  var l = item.innerText = task.description;
  console.log('task: '+l);
...
function getTasks(callback) {
  var req = new XMLHttpRequest();
  req.open('GET', '/todo/list');
  req.send();

  req.addEventListener('load', () => {
    var results = JSON.parse(req.responseText);
    // console.log(results.description)
    // console.log(results.todo[0].description)
    if (results.error) return console.log(results.error);
